...
<head>
  ...
  <body>
    ...
    <form method="GET" action="<?php echo site_url('type/manage') ?>" id="form1" class="form-inline definewidth m20">
   
        
  
        &nbsp;&nbsp;类别名称:<input type="text" name="name" class="input-default">
            ...
            <?php foreach($list as $v) :  ?>
                    ...
                    <tr>
                      ...
                      <td>
                          ...
                          <a href="<?php echo site_url('type/edit').'?tid='.$v['tid']; ?>">编辑</a>
                          <a href="javascript:void(0)" data-id="<?php echo $v['tid'] ?>" onClick="del(this)">删除</a>
                      </td>
                    </tr>
  ...
  <script>
      ...
      function del(obj)
      {
        if(confirm("确认删除么?"))
        {
          $.ajax({
            type:'get',
            url:'<?php echo site_url('Type/deleteRow'); ?>',
            datatype:'json',
            data:{tid:obj.dataset.id},
            success:function(data)
            {
              if(data == 1 || data == "1")
              {
                  window.location.href = "<?php echo site_url('type/manage'); ?>"
              }
              else
              {
                alert('删除失败!');
